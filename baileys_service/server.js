...
app.get('/groups/:instanceId', async (req, res) => {
    ...
    try {
        ...
        // Get chats from socket store
        const store = instance.sock.ev;
        const chatsStore = store.chats || {};
        
        // Filter for groups (IDs ending with @g.us)
        const allChats = Object.values(chatsStore);
        const groups = allChats.filter(chat => chat.id && chat.id.endsWith('@g.us'));
        
        console.log(`📊 ${groups.length} grupos encontrados na instância ${instanceId}`);
